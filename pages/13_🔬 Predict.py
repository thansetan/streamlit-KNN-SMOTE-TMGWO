    ...
    if data.columns[0] != "Age":
            ...
            if features[i] in int_features:
                ...
                with col2:
                    features[i] = st.number_input(
                        f"{no}. {name_dict[feature_labels[i]]}",
                        min_value=0,
                        max_value=100,
                        value=30,
                        step=1,
                ...
                with col2:
                    features[i] = st.radio(
                        f"{no}. {name_dict[feature_labels[i]]}",
                        options=reversed(sorted(data[features[i]].unique())),
                        key=features[i],
                        index=1,
                        horizontal=True,
        ...
        with col10:
            if prediction is not None:
                col10.text(
                    f"Prediksi: {'Beresiko Terkena Diabetes' if prediction[0]==1 else 'Tidak Beresiko Terkena Diabetes'}"
                )
            else:
                pass
else:
